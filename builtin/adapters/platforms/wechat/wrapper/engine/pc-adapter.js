...
function adaptMouseEvent () {
    ...
    function registerMouseEvent (funcName, engineEventType, handler) {
        wx[funcName](res => {
            let mouseEvent = inputMgr.getMouseEvent(res, window.canvasRect, engineEventType);
    ...
    registerMouseEvent('onMouseDown', EventMouse.DOWN, function (res, mouseEvent) {
        ...
        inputMgr.handleTouchesBegin([inputMgr.getTouchByXY(res.x, res.y, window.canvasRect)]);
    });
    registerMouseEvent('onMouseUp', EventMouse.UP, function (res, mouseEvent) {
        inputMgr._mousePressed = false;
        inputMgr.handleTouchesEnd([inputMgr.getTouchByXY(res.x, res.y, window.canvasRect)]);
    });
    registerMouseEvent('onMouseMove', EventMouse.MOVE, function (res, mouseEvent) {
        inputMgr.handleTouchesMove([inputMgr.getTouchByXY(res.x, res.y, window.canvasRect)]);
